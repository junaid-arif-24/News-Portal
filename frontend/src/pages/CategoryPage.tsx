...
const CategoryPage: React.FC = () => {
  ...
  const fetchSubscribedCategories = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/categories/subscribed-categories`, {
        headers: {
        ...
        <div className="flex flex-wrap -mx-2">
          {categories.map((category) => (
            <div key={category._id} className="w-full md:w-1/2 lg:w-1/3 px-2 mb-4">
                    <div onClick={() => navigate(`/all-news`, { state: { category: category.name } })} className="rounded-lg cursor-pointer shadow-md bg-white p-4 flex flex-col justify-between">
